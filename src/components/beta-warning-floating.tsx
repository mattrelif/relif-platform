...
export function BetaWarningFloating({ dict }: BetaWarningFloatingProps) {
    ...
    useEffect(() => {
        // Check if user has seen the warning for this login session
        const currentLoginTime = localStorage.getItem('r_ud'); // This is the login token/timestamp
        const lastWarningShown = localStorage.getItem('relif-beta-warning-last-shown');
        
        // Show warning if:
        // 1. User is logged in (has token)
        // 2. Warning hasn't been shown for this login session
        if (currentLoginTime && (!lastWarningShown || lastWarningShown !== currentLoginTime)) {
            setShowWarning(true);
            // Mark that warning has been shown for this login session
            localStorage.setItem('relif-beta-warning-last-shown', currentLoginTime);
        }
    }, []);
